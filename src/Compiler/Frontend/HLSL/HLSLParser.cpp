    ...
    GetReportHandler().PushContextDesc(ast->ToString());
    {
        ...
        for (auto& stmnt : ast->localStmnts)
        {
            ...
            case AST::Types::VarDeclStmnt:
                {
                    auto s = std::static_pointer_cast<VarDeclStmnt>(stmnt);
                    for (auto& d: s->varDecls)
                        d->structDeclRef = ast.get();
                    ast->varMembers.push_back(s);
                }
                break;
            case AST::Types::BufferDeclStmnt:
                {
                    auto s = std::static_pointer_cast<BufferDeclStmnt>(stmnt);
                    for (auto& d: s->bufferDecls)
                        d->structDeclRef = ast.get();
                    ast->bufMembers.push_back(s);
                }
                break;
            case AST::Types::SamplerDeclStmnt:
                {
                ...
                break;
            case AST::Types::BasicDeclStmnt:
                {
                    auto basicDeclStmnt = stmnt->As<BasicDeclStmnt>();
                    if (basicDeclStmnt->declObject->Type() == AST::Types::FunctionDecl)
                    {
                        auto d = std::static_pointer_cast<FunctionDecl>(basicDeclStmnt->declObject);
                        d->structDeclRef = ast.get();
                        /* Store copy in member function list */
                        ast->funcMembers.push_back(d);
                    }
                    else
